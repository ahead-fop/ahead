...
// *********************** TAG PARSING   **********************************/   
  ...
  // *******************************************************************************   
   ...
   public static SeeTagJTS ParseString(Doc Document, StringTokenizer tokens)
   {
	   ...
	   // Sets the inline tags to an empty array
       seeTag.setInlineTags(new TagJTS[0]);    
   ...
   public static SeeTagJTS ParseMember(Doc Document, String stringTag)
   {
	   ...
	   String memberName = "";      // first token after #
	   
	   // Creates the StringTokenizer
	   String delims = " #(),.";
	   StringTokenizer tokens = new StringTokenizer(stringTag,delims, true);
	   
	   // Reads all leading whitespaces
	   while(tokens.hasMoreTokens()) 
		   if (tokens.nextToken().compareTo(" ")!=0) break;
	   
	   // Keeps reading until it finds a # character or end of string
       while(tokens.hasMoreTokens())
	   {
		   token = tokens.nextToken();
	   ...
	   System.out.println("PClass -> " + packageClass + " memberName-> " + memberName);
	   ...
	   // Sets the First sentence array to be the text of the string.
	   TagJTS[] arrayTags = new TagJTS[1];
	   arrayTags[0] = new TagJTS(Document, packageClass, "Text");
	   arrayTags[0].setFirstSentenceTags(new TagJTS[0]);
	   arrayTags[0].setInlineTags(new TagJTS[0]);
	   seeTag.setFirstSentenceTags(arrayTags);   	   
	   ...
	   // Pending --> MemberDoc, Package, ClassDoc setting. 
	   return seeTag;
   }
   
   /** Parses the Member description, that is, whatever is after the #
    *  Current implementation returns only an string, but further versions
    *  should return more...
    */
