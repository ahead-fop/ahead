...
AST_Program
	...
	|LOOKAHEAD( [ LayerModifiers() ] <LAYER>) 
	  Layer_Decl	::LDecl
	| [ PackageDeclaration ] [ AST_Imports ] [ AST_Class ]	::program
	;


TE_Generator
	: TYPEEQN AST_QualifiedName AST_QualifiedName "=" AST_Exp ";"	:: TEGen
	;


LayerHeader
	: [ LayerModifiers ] LAYER QName "(" [ AST_ParList ] ")"	::LyrHeader
	;
...
MethodDeclaration
	: [ AST_Modifiers ] AST_TypeName MethodDeclarator [ ThrowsClause ] MethodDeclSuffix	::MethodDcl
	;
...
ConstructorDeclaration
	: [ AST_Modifiers ] QName "(" [ AST_ParList ] ")" [ ThrowsClause ] "{" [LOOKAHEAD(ExplicitConstructorInvocation()) 
...
ExplicitConstructorInvocation
	...
	| [LOOKAHEAD(2)  PrimDot ] "super" Arguments ";"	::ConSuper
	;
...
SwitchStatement
	:  "switch" "(" Expression ")" "{" [ AST_SwitchEntry ] "}"	::SwitchStmt
	;
...
IfStatement
	: "if" "(" Expression ")" Statement [LOOKAHEAD(1)  ElseClause ]	::IfStmt
	;
