...
public class UnmixinThread extends Thread
{
	private Main me;
	...
	public UnmixinThread(Main me,ToolsPanel cp, boolean isFile, DefaultMutableTreeNode node){
			this.me = me;
			this.cp = cp;
			this.isFile = isFile;
			this.node = node;
	...
	public UnmixinThread(Main me,ToolsPanel cp, boolean isFile){
		this.me = me;
		this.cp = cp;
		this.isFile = isFile;
		this.node = null;
	...
	public void run(){
		...
		cp.outputArea.append("Finished running the unmixin process.");
		cp.outputArea.append("\n");
		try{
			cp.outputArea.setCaretPosition(cp.outputArea.getDocument().getLength());
		}catch(Exception be){}
		me.status.setText("Status: Finished running the unmixin.");
		me.status.setForeground(me.editorTitle.getForeground());
		cp.setEnabled(true);
	}

	public void invokeUnmixin(String filePath){
		try{
			if (filePath.endsWith(File.separator)){
				filePath=filePath.substring(0,filePath.length()-1);
			}
			...
			else if( osName.equals( "Windows 95" ) )
			{
				...
				pr=Runtime.getRuntime().exec(execStr.toString(),null,new File(me.modelDir));
                        ...
                        String[] cmd = new String[]{"env", "--", "unmixin", filePath};
			...
			if (status != 0){
				cp.outputArea.append("Error occured during running the unmixin.\n");
				try{
					cp.outputArea.setCaretPosition(cp.outputArea.getDocument().getLength());
				}catch(Exception be){}
			}
