/* ******************************************************************
   class      : JavaSyntaxColoring
   description: A syntax highlighter for the java syntax.
*********************************************************************/
...
public class JavaSyntaxColoring extends AbstractSyntaxColoring {

  // class variable declarations
  private final static String KEYWORD_PREFIX    = "keywords";
  private final static String SPECIALWORD_PREFIX = "specialwords";
  ...
  public JavaSyntaxColoring() {
    ...
    commentBuffer.clear(0);
    instCnt++;
  } // end constructor JavaSyntaxColoring
    ...
    if (isToken(ch)) {
      ...
      if (isKeyword(array,start-1,scanned))
        prefetchedType = KEYWORD;
		  ...
		  if (isSpecialword(array, start-1, scanned))
			   prefetchedType = SPECIALWORD;
		  else
			   prefetchedType = NORMAL_TEXT;
	  }
      // end if
  ...
  //load the special words.
   protected char[][] loadSpecialwords() {
    Vector vector = new Vector();
    int i         = 1;
    String words  = getProperty(SPECIALWORD_PREFIX + 1);
    String str;
    StringTokenizer t;
    char[][] result;
    ...
    while (words != null) {
      ...
      i++;
      words = getProperty(SPECIALWORD_PREFIX+i);
    } // end while
    result = new char[vector.size()][];
    for (i=0; i<result.length; i++)
      result[i] = ((String)vector.elementAt(i)).toCharArray();
  ...
  protected boolean isKeyword(char[] cbuf, int offs, int len) {
   ...
   protected boolean isSpecialword(char[] cbuf, int offs, int len) {
    int cur,l,r,result;

    if (len < 2) return false;
    // binary search
    l = 0; r = specialwords.length-1;
    do {
      cur    = (l + r) / 2;
      result = match(cbuf,offs,len,specialwords[cur]);
      if ((result < 0) || (l > r))
        r = cur-1;
      else
        l = cur+1;
