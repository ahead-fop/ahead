...
public class SafeCompositionValidator {
    ...
    static boolean OUTPUT_FAILURES = true;
    
    static boolean DEBUG_OUTPUT=false;
    static boolean FLIP = true;
    static boolean CHECKSELFREFERENCES=false;

    static String DollarDollar="$$";              // this is the stupid extension used in the bytecode composition tools
                    ...
                    while(defIter.hasNext()){
                        ...
                        for (int i=0;i<subClasses.size();i++){
                            ...
                            if (numDefs==0){
                                ...
                                System.out.println("Zero Definitions of class: "+ concreteClass);
    ...
    */
    ...
    void ReferenceFieldConstraint(FOPImplementationInfo fopimpl){
            ...
            for(Iterator methodIter = plclass.getListOfMethods().iterator(); methodIter.hasNext(); )
            {
                ...
                while(refIter.hasNext()){
                    ...
                    if (!fopimpl.getOODesigninfo().isProductLineClass(fieldsClassName)){
                        System.out.println("Skipping "+fieldsClassName);
                        continue;
                    }
                    ...
                    //System.out.println("looking for "+fieldId + " in class "+fieldsClassName);
                    ...
                    //System.out.println(implDetails +" "+layerDetails);
        ...
        //System.out.println("Number of field ref constraints:"+numConstraints);
...
//                  Inherited fields can be referenced too.. so if they dont exist we need to look up
    ...
    void ReferenceMethodConstraint(FOPImplementationInfo fopimpl){
        ...
        for(Iterator classIter = fopimpl.getListOfClasses().iterator(); classIter.hasNext(); )
        {
            ...
            for(Iterator methodIter = plclass.getListOfMethods().iterator(); methodIter.hasNext(); )
            {
                ...
                while(refIter.hasNext()){
                    ...
                    if (!fopimpl.getOODesigninfo().isProductLineClass(methodsClassName)){
                        //System.out.println("Skipping "+methodsClassName);
                        continue;
                    }
                    //find the class, then PLField, then layer
                    PLClass methodsClass = fopimpl.getClass(methodsClassName);
                    ...
                    //System.out.println(implDetails);
        ...
        //System.out.println("Number of method ref constraints:"+numConstraints);
    }
    /**
    ...
    void ReferenceClassConstraint(FOPImplementationInfo fopimpl){
        ...
        //System.out.println("Number of class ref constraints:"+numConstraints);
        ...
        if (path.isFile()) {
	    	...
	    	if (path.getAbsolutePath().endsWith(".class")){
	    	    ...
	    	    if(path.getName().matches("[^\\$]++\\$++[0-9]++\\.class")){
	    	        //System.out.println("Skipping.. "+path.getName());
	    	        return;
	    	    }
		...
		while(!nodeStack.empty()){
			...
			while(it.hasNext()){
				...
				if (candidate.getName().equals("file") && candidate.getAttributeValue("type").equalsIgnoreCase("class")){
				    ...
				    if(candidate.getAttributeValue("name").matches("[^\\$]++\\$++[0-9]++\\.class")){
		    	        //System.out.println("Skipping.. "+candidate.getAttributeValue("name"));
		    	        //continue;
		    	    }
				    else {
				        //System.out.println(candidate.getAttributeValue("name"))
				        //System.out.println(candidate.getAttributeValue("location"));
				    }
    ...
    Element readPLM(String plmfilePath){
        try {
			...
			if (plm.errorMessage.length()>0){
				System.out.println("There was an error reading plm file");
			...
			for(int i=0;i<features.size();i++){
				...
				try {
				    ...
				    System.out.println("-----------");
				    System.out.println("File: "+plm.getRootPath()+"\\"+feature.name);
...
//      Defaults
        ...
        try {
            ...
            for (int i=0;i<args.length-endParams;i++){
                ...
                if (args[i].equals("-f")||args[i].equals("-failures"))
                    ...
                    DEBUG_OUTPUT=true;
                else if (args[i].equals("-s")||args[i].equals("-nostats"))
                    REPORT_STATS=false;
            ...
            System.out.println("  -s  Do Not Report Validation Statistics");
            System.out.println("  -ie Treat Introduction Constraints as Error (default)");
        ...
        if (!multidimensional) 
            validator.validateLinearStandAlone(args[args.length-2] + DollarDollar, args[args.length-1]);
        else
    ...
    boolean checkRank(Vector<Coordinate> left, Vector<Coordinate> right){
        ...
        for (int i=0;i< dimensions;i++){
            ...
            if (!((!flip && (leftrank <= rightrank)) ||
            	  (flip && (leftrank >= rightrank))))
            {
                
                System.out.println("Ranking constraint violated");
                
                //System.out.println(realLeft + " after " +realRight);
                System.out.println("\t" + getMaxRankLayer(realLeft)+" (position " +leftrank + ") is before " +getMinRankLayer(realRight)+" (position "+rightrank+") in the feature model, and not after as required");
                return false;
            }
