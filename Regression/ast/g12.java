...
// and the vlst{ ... }vlst and $vlst() escape
...
class g12 {
   ...
   AST_VarDecl vlst =(AST_VarDecl) AstNode.markStack(AstNode.aliasStack.size(),  (AST_VarDecl) new AST_VarDecl()
...
.add( (AST_StmtElem) new AST_StmtElem().setParms( (VEStmt) new VEStmt().setParms( 
 (LocalVarDecl) new LocalVarDecl().setParms( 
 new AstOptToken(
) /* AstOptToken */
,  (QNameType) new QNameType().setParms( 
...
.add( (AST_VarDecl) AstNode.addComment( AstNode.safeCopy(vlst)," ")) 
) /* LocalVarDecl */
, new AstToken().setParms("",";", 0)) /* VEStmt */
))/* AST_StmtElem + add */
...
.add( (AST_QualifiedNameElem) new AST_QualifiedNameElem().setParms( (NameId) new NameId().setParms(new AstToken().setParms("   // <-------------- fill in escape here\n\n      // Step 1: initialize the directory\n\n      ","directory", 0)) /* NameId */
))/* AST_QualifiedNameElem + add */
) /* PPQualName */
