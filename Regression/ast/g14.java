...
// and the estm{ ... }estm and $estm() escapes
...
class g14 {
   ...
   AST_Catches cat =(AST_Catches) AstNode.markStack(AstNode.aliasStack.size(),  (AST_Catches) new AST_Catches()

.add( (AST_CatchesElem) new AST_CatchesElem().setParms( (CatchStmt) new CatchStmt().setParms( 
new AstToken().setParms(" ","catch", 0), new AstToken().setParms(" ","(", 0),  (FormParDecl) new FormParDecl().setParms( 
 new AstOptToken(
) /* AstOptToken */
...
.add( (AST_CatchesElem) new AST_CatchesElem().setParms((AST_Catches) AstNode.addComment( AstNode.safeCopy(cat),"\n       ")))/* AST_CatchesElem + add */
) /* AstOptNode */
,  new AstOptNode(
) /* AstOptNode */
) /* TryStmt */
...
.add( (AST_StmtElem) new AST_StmtElem().setParms( (Empty) new Empty().setParms( 
new AstToken().setParms("",";", 0)) /* Empty */
))/* AST_StmtElem + add */
...
.add( (AST_QualifiedNameElem) new AST_QualifiedNameElem().setParms( (NameId) new NameId().setParms(new AstToken().setParms("    //<-------- escape here\n       \n       // Step 4: create print stream; output makefile contents and close\n\n       ","ps", 0)) /* NameId */
))/* AST_QualifiedNameElem + add */
) /* PPQualName */
,  (Assign) new Assign().setParms( 
new AstToken().setParms(" ","=", 0)) /* Assign */
...
.add( (AST_ArgListElem) new AST_ArgListElem().setParms( (StrLit) new StrLit().setParms( 
new AstToken().setParms("","\"\tjava -mx32m java_cup.Main -expect 8 -parser $(Prj)Parser -symbols JakartaSym <Jakarta.cup\"", 0)) /* StrLit */
))/* AST_ArgListElem + add */
) /* AstOptNode */
, new AstToken().setParms("",")", 0)) /* Args */
) /* MthCall */
