...
// it also tests the stm{ ... }stm and $stm() escape
...
class g4 {
   ...
   AST_Stmt stm =(AST_Stmt) AstNode.markStack(AstNode.aliasStack.size(),  (AST_Stmt) new AST_Stmt()
...
.add( (AST_Stmt) AstNode.addComment( AstNode.safeCopy(stm),"\n      ")) 
) /* AstOptNode */
, new AstToken().setParms("                //<-------fill in statement here\n   ","}", 0)) /* BlockC */
) /* MDSBlock */
) /* MethodDcl */
))/* AST_FieldDeclElem + add */
