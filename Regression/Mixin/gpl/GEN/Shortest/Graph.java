...
refines class Graph {
      ...
      public Graph ShortestPath( Vertex s) { Vertex source;
		
	source = s;
	int numvertices = vertices.size();
	int i; Vertex x,n; Neighbor vn;
	int wuvn;
	int k1;
	...
	Queue.addFirst(source); Vertex ucurrent;
	int j,k,l;
	int pos;
	LinkedList Uneighbors; Vertex u,v; Edge en;
	int wuv;
	...
	while (Queue.size()!=0)
	{
	 ...
	 theName = (( Vertex)vertices.get(i)).name;			
       ...
       } Vertex theVertex, thePred; Vertex theNewVertex, theNewPred; Edge   e; Neighbor theNeighbor,newNeighbor;
       boolean flag = false;
          ...
          do
          {
             ...
             if (theNeighbor.end.name.equals(theVertex.name))
              flag = true;
               ...
               j++;
          }while(flag==false && j< thePred.neighbors.size()); Edge theNewEdge = new Edge(theNewPred, theNewVertex);
         e = theNeighbor.edg;
